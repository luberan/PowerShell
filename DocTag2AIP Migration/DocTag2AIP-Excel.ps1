...
msg * "Reklasifikace Excel dokumentů začala."
$time=Get-Date
"$time Reklasifikace Excel dokumentů začala." | Out-File C:\reklasifikace\statusExcel.txt -Append

# Otviram Excel na pozadi
...
foreach ($folder in $folders) {
    if (($folder.Name -ne "Program Files") -and ($folder.Name -ne "Program Files (x86)") -and ($folder.Name -ne "PerfLogs") -and ($folder.Name -ne "ProgramData") -and ($folder.Name -ne "Windows")) {
        Write-Host $folder
        Try {
            $files += Get-ChildItem -Path $folder.Fullname -Include @("*.xls", "*.xlsx") -Recurse
        }
        Catch {
            $time=Get-Date
            "$time $file $PSItem" | Out-File C:\reklasifikace\errorExcel.txt -Append
        }
    }
}
if (Test-Path D:) {
    $files += Get-ChildItem -Path D:\ -Include @("*.xls", "*.xlsx") -Recurse
}
...
foreach ($file in $files) {
    ...
    if (($protected.IsLabeled -eq $false) -and ($protected.IsRMSProtected -eq $false)) {
        ...
        $doc = $excel.workbooks.open("$file.Fullname",$False,$False,$False,"NeplatneHeslo","",$False,"","",$wdOpenFormat,$msoEncoding,$False,$False,$False)
        $labels = @()
        ...
        foreach($lbl in $labels) {
            Set-AIPFileLabel -Path $file.Fullname -LabelId $lbl -PreserveFileDetails | Out-File C:\reklasifikace\aipExcel.txt
        }
...
msg * "Reklasifikace Excel dokumentů byla dokončena."
$time=Get-Date
"$time Reklasifikace Excel dokumentů byla dokončena." | Out-File C:\reklasifikace\statusExcel.txt -Append
